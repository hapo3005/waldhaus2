(() => {
  ...
  const demoBookings=()=>{const a=plus(new Date(),1),b=plus(new Date(),12);return[
    {id:'demo-becker',guest:'Familie Becker',guests:4,start:iso(a),end:iso(plus(a,3)),source:'Direkt',status:'confirmed'},
    {id:'demo-schmitz',guest:'Familie Schmitz',guests:2,start:iso(b),end:iso(plus(b,4)),source:'Booking',status:'confirmed'}
  ];};
  const demoRequests=()=>{const a=plus(new Date(),22);return[{id:'demo-weber',guest:'Familie Weber',email:'familie.weber@example.de',phone:'+49 170 1234567',guests:3,start:iso(a),end:iso(plus(a,3)),note:'Wochenende in der Eifel',status:'pending'}];};
  ...
  const isPending=x=>(x?.status||'pending')==='pending';
  const openRequests=()=>state.requests.filter(isPending);
  ...
  function inject(){
    ...
    const link=document.createElement('link');link.rel='stylesheet';link.href='owner-ops.css?v=2';document.head.appendChild(link);
    const stats=dash.querySelectorAll('.owner-stat');
    if(stats[0]){stats[0].querySelector('strong').id='ownerStayCount';stats[0].querySelector('span').textContent='bestätigt';}
    if(stats[1]){stats[1].querySelector('small').textContent='Offene Anfragen';stats[1].querySelector('strong').id='ownerRequestCount';stats[1].querySelector('span').textContent='noch unverbindlich';}
    ...
    const s=document.createElement('section');s.id='ownerOps';s.className='content-section owner-ops';s.innerHTML=`
      <div class="owner-ops-head"><span class="card-label">Übersicht</span><h2>Belegung & Anfragen.</h2><p>Nur bestätigte Aufenthalte blockieren den Kalender. Offene Anfragen bleiben unverbindlich und dürfen sich überschneiden.</p></div>
      <div class="owner-ops-grid">
        <article class="card owner-calendar-card"><div class="owner-card-head"><div><span class="card-label">Belegung</span><h3 id="ownerCalendarTitle"></h3></div><div><button id="ownerPrevMonth" aria-label="Vorheriger Monat">‹</button><button id="ownerNextMonth" aria-label="Nächster Monat">›</button></div></div><div class="owner-weekdays"><span>Mo</span><span>Di</span><span>Mi</span><span>Do</span><span>Fr</span><span>Sa</span><span>So</span></div><div id="ownerCalendar" class="owner-calendar"></div><div class="owner-legend"><span><i class="booked"></i>Bestätigt / belegt</span><span><i class="request"></i>Offene Anfrage</span></div><p class="owner-status-help">Eine Anfrage reserviert noch nichts. Erst mit „Bestätigen“ wird daraus eine Belegung.</p></article>
        ...
        <article class="card"><span class="card-label">Offene Anfragen</span><h3>Prüfen, bestätigen oder ablehnen.</h3><div id="ownerRequestList" class="owner-list"></div></article>
  ...
  function renderLists(){
    ...
    const b=upcoming().slice(0,5);
    bl.innerHTML=b.length?b.map(x=>`<article class="owner-item"><div><strong>${safe(x.guest)}</strong><span>${fmt(x.start)}–${fmt(x.end)} · ${nights(x)} Nächte · ${x.guests} Pers.</span></div><div><small>Bestätigt · ${safe(x.source||'Direkt')}</small><button data-del-booking="${safe(x.id)}" aria-label="Aufenthalt entfernen">×</button></div></article>`).join(''):'<p class="owner-empty">Noch keine kommenden Aufenthalte.</p>';

    const requests=openRequests();
    rl.innerHTML=requests.length?requests.map(x=>{
      const conflict=overlap(x.start,x.end);
      return `<article class="owner-item owner-request-item${conflict?' has-conflict':''}"><div><div class="owner-request-title"><strong>${safe(x.guest)}</strong><small class="owner-request-state ${conflict?'conflict':'open'}">${conflict?'Zeitraum belegt':'Offen'}</small></div><span>${fmt(x.start)}–${fmt(x.end)} · ${x.guests} Pers.</span>${requestContact(x)}${x.note?`<small class="owner-request-note">${safe(x.note)}</small>`:''}${conflict?'<small class="owner-request-conflict">Eine bestätigte Buchung überschneidet sich inzwischen mit dieser Anfrage.</small>':''}</div><div><button class="confirm" data-confirm="${safe(x.id)}" ${conflict?'disabled title="Zeitraum bereits belegt"':''}>${conflict?'Nicht möglich':'Bestätigen'}</button><button class="decline" data-del-request="${safe(x.id)}">Ablehnen</button></div></article>`;
    }).join(''):'<p class="owner-empty">Keine offenen Anfragen.</p>';
    ...
    document.querySelectorAll('[data-del-request]').forEach(b=>b.onclick=()=>{state.requests=state.requests.filter(x=>x.id!==b.dataset.delRequest);persist();render();toast('Anfrage abgelehnt');});
    document.querySelectorAll('[data-confirm]').forEach(b=>b.onclick=()=>{if(b.disabled)return;const r=state.requests.find(x=>x.id===b.dataset.confirm);if(!r)return;if(overlap(r.start,r.end))return toast('Zeitraum ist inzwischen belegt');state.bookings.push({...r,status:'confirmed',source:'Direkt'});state.requests=state.requests.filter(x=>x.id!==r.id);persist();render();toast('Anfrage bestätigt – Zeitraum jetzt belegt');});
  }
  ...
  function syncGuest(){
    ...
    const pill=document.querySelector('.status-pill');if(pill)pill.innerHTML='<i></i> Aufenthalt bestätigt';
  }

  function render(){const a=document.querySelector('#ownerStayCount'),r=document.querySelector('#ownerRequestCount');if(a)a.textContent=upcoming().length;if(r)r.textContent=openRequests().length;renderCalendar();renderLists();syncGuest();}
